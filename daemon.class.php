<?php
/**
 * @author Ian Maddox ian@isnoop.net
 ...
 * @version 1.1
 *
 * This script is a PHP daemon pattern controller.  It provides the ability to run multiple concurrent instances
 * for a daemon.  It uses a PID file stored in the /tmp directory by default.
 *
 * When conflicts are found, the results are returned in a non-fatal form.  It is up to the daemon
 * to read the responses to start() and heartbeat() and act accordingly.
 *
 * If a TRUE response is returned after calling start(), the daemon *must* call stop() before execution ends.
 *
 * This code also captures SIGTERM, SIGHUP, and SIGINT.  The next heartbeat after one of these signals
 * is received will tell the daemon to halt.
 */
class Daemon {
   private $fp;
   private $data;
    ...
    *
    * @return boolean FALSE indicates PID not registered.  Daemon should quit.
    */
